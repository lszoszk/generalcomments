...
| v8 | `3301aba` | 2026-04-28 | Metadata audit and schema enrichment. Phase 1 fixes: cast all `Adoption Year` to int (39 GC + 65 SP), merged SP `Adoption year` lowercase variant, fixed CCPR GC31 signature typo, removed orphan CRC-GC18-Harmful (non-Rev), standardised 18 outlier links to canonical `tbinternet.ohchr.org/Download.aspx`. Phase 2 adds: `paragraphCount`, `wordCount`, `labelCount`, `ohchrSymbol`, `firstAddedAt`, `lastVerifiedAt`, `articles`, `status`, `supersedes`/`supersededBy`, `jointWith`, `languagesAvailable`, `alternativeSignatures`. Phase 3 adds: hand-written one-sentence Committee-voice `abstract` for all 186 GCs. `topicTags` deferred to backlog (TODO_LATER.md). Net: GC count 187 → 186, schema gains 12 optional fields. |
| v9 | `4131eb1` | 2026-04-28 | Make new metadata actually filterable. SP backfill: filled 88 missing `Mandate holder` for SR Freedom of Religion or Belief (curated year-based map: d'Almeida Ribeiro / Amor / Jahangir / Bielefeldt / Shaheed / Ghanea), added `reportType` to all 151 SP records (annual / thematic / communications / addendum / country-visit), parsed `hrcSession`/`gaSession` from signatures (112 records), synthesized `Presented` for 85 records that lacked it. New filter UI on the website: Treaty articles (71 unique values, searchable chip grid sorted numerically), SP report type (only visible when scope is SP/All), and a "Show superseded GCs" toggle (defaults to hiding the 2 superseded records). All filter state survives in URL query (`art=`, `rt=`, `sup=`). |
| v10 | `83c4df5` | 2026-04-28 | **Operational hygiene + first SP mandate expansion.** (a) Link validator: new `validate_links.py` HEAD-checks every `Link` in metadata, updates `lastVerifiedAt` for working URLs and emits a JSON status report. Cleaned 13 outlier SP links pointing to `www.ohchr.org/.../thematic-reports/...` (HTTP 403) by switching them to canonical `docs.un.org/en/{signature}` — net result 338/338 unique URLs OK. New GitHub Actions workflow `.github/workflows/link-check.yml` runs the validator weekly (cron: Mondays 03:00 UTC), commits `lastVerifiedAt` updates, opens/updates a "link-rot" issue when any URL breaks. (b) **First SP mandate expansion:** generic `ingest_sp_mandate.py` pipeline (PDF discovery via `daccess-ods.un.org` with OHCHR fallback, PyMuPDF paragraph extraction, label application, idempotent metadata append). Test mandate: SR on the Rights of Persons with Disabilities — 22 of 23 thematic reports ingested successfully (1 skip: A/HRC/61/26 from March 2026 not yet in any UN system), 1,919 new paragraphs at ~88% label coverage. Mandate holders auto-assigned: Devandas-Aguilar (12), Quinn (6), Hagrass (4). (c) **Articles filter removed from UI** — chip values like `Art. 6` collide between treaties (ICCPR/ICESCR/CRC) and were misleading. The metadata field is preserved per-document (still shown in the dossier) but no longer aggregated as a sidebar facet. Net: 332 → 359 documents, 23,937 → 25,856 paragraphs. |
| v11 | (this commit) | 2026-04-28 | **Search overhaul** — feature-parity-light with the SQLite FTS5 backend used by the reference app `lszoszk/UnitedNations_recommendations`, but kept fully client-side. Three commits (`P1` `97536af`, `P2` `e74e1ef`, `P3` this one): (a) Recursive-descent boolean parser supporting `"phrase"`, `AND`, `OR`, `NOT` (also `&`, `\|`, `-`, `!`), parentheses, and prefix wildcards (`bias*` → biased / biasing / biases). The parser produces an AST evaluated against the FlexSearch index; NOT branches are skipped during highlight to avoid marking excluded terms. (b) KWIC (keyword-in-context) snippets ported from `dashboard-search.js`: when the keyword falls past the ~400-char fold of a long paragraph the result card shows a 1–2-sentence excerpt centred on the densest cluster of query terms, with an `◎ KWIC · N chars` badge. (c) Hard 200-result cap replaced by IntersectionObserver pagination — 50 paragraphs per page, hard safety cap at 5 000. (d) Score replaced by BM25-lite with per-scope IDF and document-length normalisation (k₁=1.5, b=0.75, smoothed IDF). Rare terms now outrank frequent ones; short paragraphs with N hits beat long paragraphs with the same N. (e) Tailored empty-state card surfaces query-vs-filters cause + recovery buttons + inline syntax cheatsheet. (f) Inline `×` clear chip on the search input (also bound to Esc). (g) `GC N ¶ · SP N ¶` breakdown pills next to the result count when scope is "All sources". |
